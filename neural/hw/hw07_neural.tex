...
\[
      J = \sum_{i=1}^N\left[ \ln(1+e^{y_iz_i}) - y_iz_i \right], \quad z_i = \sum_{j=1}^p x_{ij}w_j + b.
\]
\begin{enumerate}[(a)]
...
\[
    \frac{\partial J}{\partial z_i} = -y_i + \frac{y_ie^{y_iz_i}}{1+e^{y_iz_i}}.
\]
...
\beq \label{eq:gloss_bin}
    g_{\rm loss}(z^\out_i,y_i) = \ln\left[ 1 + e^{y_iz_i^\out} \right] -y_iz_i.
\eeq
